...
// toolCall is the wire shape the model emits per toolProtocol.
...
func parseToolCall(reply string, allowed map[string]bool) (*genai.FunctionCall, bool) {
	...
	if tc.Tool == "" || !allowed[tc.Tool] {
		return nil, false
	}
	if tc.Args == nil {
		tc.Args = map[string]any{}
	}
	return &genai.FunctionCall{Name: tc.Tool, Args: tc.Args}, true
}
...
// hasFunctionResponse reports whether any content already carries a tool result.
// The tool bridge uses this to stop offering tools after one round-trip, which
// bounds a weak local model to a single tool call instead of looping.
